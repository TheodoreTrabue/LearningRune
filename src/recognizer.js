...
import { smallDetails } from './details.js';
...
export function recognize(strokes, templates) {
  ...
  const initial = [...scores].sort((a, b) => a[1] - b[1]);
  const pair = initial.slice(0, 2).map(([letter]) => letter).sort().join('');
  // Only disambiguate an already plausible pair. Details cannot turn an
  // unrelated shape into a letter, and the ordinary rejection gates still apply.
  if (initial[0]?.[1] < 0.085 && (pair === 'IW' || pair === 'AE')) {
    const details = smallDetails(cloud);
    let penalized = null;
    if (pair === 'IW') {
      if (details.tail > 0.07) penalized = 'I';
      else if (details.tail !== null && details.tail < 0.025) penalized = 'W';
    } else {
      if (details.bar >= 5 / 6) penalized = 'A';
      else if (details.bar <= 0.5) penalized = 'E';
    }
    if (penalized) scores.set(penalized, scores.get(penalized) + 0.05);
  }
